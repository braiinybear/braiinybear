...
const Registration: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (marksheetResults.length === 0 && formData.marksheets.length > 0) {
        ...
        for (let i = 0; i < formData.marksheets.length; i++) {
          ...
          setUploadProgress(
            `Uploading marksheet ${i + 1} of ${formData.marksheets.length}...`
          );
   ...
   console.log(errorMessage);
      ...
      // Optional: still keep local state if needed
      setSubmitStatus({
        type: "error",
  ...
  return (
    <div className="bg-gradient-to-br from-blue-50 to-indigo-100 min-h-screen py-12 px-4 sm:px-6 lg:px-8">
      <Helmet>
        <title>
          Online Registration | BraiinyBear Educational and Training Society
        </title>
        <meta
          name="description"
                      ...
                      <div className="mt-3 space-y-3">
                        {uploadedImages.marksheets.map((marksheet, index) => (
                          ...
                          >
                            ...
                            {formData.marksheets[index]?.type ===
                            "application/pdf" ? (
                              <a
                                href={marksheet.url}
                                target="_blank"
